...
# Limit visible GPUs so Lightning stays single-device and doesn't spawn DDP workers.
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-2}"
export YAIB_TRAINER_DEVICES="${YAIB_TRAINER_DEVICES:-3}"
...
run_hospital() {
   ...
   PATH="$PWD/.venv/bin:$PATH" uv run --python 3.11 python -m icu_benchmarks.run \
        ...
        -m K29 \
        --cpu \
        -hp model/hyperparameter.n_rff_features=740 \
            model/hyperparameter.gamma=0.025
}
