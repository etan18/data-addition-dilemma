...
class MondrianForest:
    ...
    - Continuous features are mapped with a Gaussian kernel (RFF)
    """
    ...
    def _transform_continuous(self, z):
        if self.rff is None:
            raise ValueError("Kernel features are not initialized. Call fit first.")
        if z.ndim == 1:
            return self.rff.transform(z[None, :])[0]
        return self.rff.transform(z)
    ...
    def fit(self, X, y):
        ...
        z_fit = z if z.ndim > 1 else z[None, :]
        self.rff = RBFSampler(
    ...
    def predict(self, X):
        ...
        z, g = self._split_features(X)
        z = self._transform_continuous(z)
        out = np.zeros(len(z))
